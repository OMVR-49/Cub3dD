...
/*   Updated: 2024/03/27 06:32:17 by sacharai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**convert_map_to_array(t_start *map)
{
	...
	maplen = list_length(map);
	maparray = ft_malloc(sizeof(char *) * (maplen + 1));
	i = 0;
	tmpmap = NULL;
	while (map)
	{
...
t_map	*initialize_map(t_start *head, t_start *map, int *rgb)
{
	...
	maparray = convert_map_to_array(map);
	help = check_map_valid_char(maparray);
